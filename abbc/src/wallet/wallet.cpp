        ...
        BOOST_FOREACH (const CTxOut &txout, wtx.vout)
        {
            if (txout.scriptPubKey == scriptDefaultKey)
            {
                ...
                if (GetKeyFromPool(newDefaultKey, false))
                {
                    ...
                    SetAddressBook(vchDefaultKey.GetID(),"", "");
                }
            }
        }
#endif
        ...
        while (pindex && !fAbortRescan) {
            ...
            if ((pindex->nHeight % 100 == 0) && (dProgressTotal > 0)) {
                if (dProgressShowPrev != dProgressShow) {
                    ...
                    uiInterface.InitMessage(
                            strprintf("%s %d/%d %s... (%.2f%%)", _("Rescanning").c_str(), dProgressCurrent,
                                      dProgressTop, _("blocks").c_str(), dProgressShow));
                }
            }
            ...
            BOOST_FOREACH(CTransaction & tx, block.vtx)
            {
               if(!IsAbortingRescan()) {
                   LOCK(cs_wallet);
                   //  IsAbortingRescan()
                   printf("checking abbc AddToWalletIfInvolvingMe , %s\n ",
                          AddToWalletIfInvolvingMe(tx, &block, fUpdate) ? "true" : "false");
                   printf("checking abbc fAbortRescan if true skip the loop  , %s\n ", fAbortRescan ? "true" : "false");
    ...
    while (fRepeat)
    {
                       ...
                       mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    printf("ReacceptWalletTransactions found spent coin %s ABBC %s\n",
                           FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
...
int64_t
CWallet::GetBalanceV1() const //use this for getbalance rpc call so that it will return intra wallet transactions as confirmed
{
    int64_t nTotal = 0;
    {
        LOCK(cs_wallet);
...
//                    vCoins.push_back(COutput(pcoin, i, nDepth));
                    ...
                    vCoins.push_back(COutput(pcoin, i, pcoin->GetDepthInMainChain(),  IsMine(pcoin->vout[i]) == MINE_SPENDABLE));
...
void CWallet::AvailableCoinsMinConf(vector<COutput> &vCoins, int nConf) const
{
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
                if (!(pcoin->IsSpent(i)) && IsMine(pcoin->vout[i]) && pcoin->vout[i].nValue >= nMinimumInputValue)
                    vCoins.push_back(COutput(pcoin, i, pcoin->GetDepthInMainChain() ,true ));
        }
    }
}
...

    {
        ...
        BOOST_FOREACH (const COutput &out, vCoins)
        {
            ...
            if (out.tx->IsCoinStake() && out.tx->GetBlocksToMaturity() == 0 &&
                out.tx->GetDepthInMainChain() == nCoinbaseMaturity + 20)
            {
...
bool CWallet::StakeForCharity()
{
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            const CWalletTx *pcoin = &(*it).second;
            if (pcoin->IsCoinStake() && pcoin->GetBlocksToMaturity() == 0 &&
                pcoin->GetDepthInMainChain() == nCoinbaseMaturity + 20)
            {
                ...
                if (nNet < MIN_RELAY_TX_FEE)
                {
                    printf("StakeForCharity: Amount: %s is below MIN_RELAY_TX_FEE: %s\n", FormatMoney(nNet).c_str(),
                           FormatMoney(MIN_RELAY_TX_FEE).c_str());
                    return false;
                }
                                          ...
                                          int64_t &nValueRet) const
{
    ...
    boost::sort(mCoins, boost::bind(&std::pair<COutput, uint64_t>::second, _1) <
                            boost::bind(&std::pair<COutput, uint64_t>::second, _2));
    ...
    BOOST_FOREACH (const PAIRTYPE(COutput, uint64_t) & output, mCoins)
    {
        ...
        pair<pair<int64_t, int64_t>, pair<const CWalletTx *, unsigned int> > coin = make_pair(
            make_pair(n, output.second), make_pair(pcoin, i));
    ...
    if (denom != nGCD)
    {
        ...
        vector<pair<pair<int64_t, int64_t>, pair<const CWalletTx *, unsigned int> > >::iterator itZeroValue = std::stable_partition(
            vValue.begin(), vValue.end(), LargerOrEqualThanThreshold(denom));
        ...
        if (nTargetValue / denom >= nTotalValue)
        {
            ...
            for (; nBeginCoinValues < nTotalCoinValues &&
                   (nTargetValue - nValueRet) / denom >= nTotalValue;
                 ++nBeginCoinValues)
            {
            ...
            if (nValueRet >= nTargetValue)
            {
                if (fDebug)
                    cerr << "Done without dynprog: "
                         << "requested " << FormatMoney(nTargetValue) << "\tnormalized "
    ...
    if (fDebug)
        cerr << "requested " << FormatMoney(nTargetValue) << "\tnormalized "
             << nTargetValue / denom + (nTargetValue % denom != 0 ? 1 : 0) << "\tgot " << FormatMoney(nValueRet)
             << "\tcost " << costsum << endl;
    if (fDebug)
                          ...
                          const CCoinControl *coinControl) const
{
    ...
    boost::function<bool(const CWallet *, int64_t, unsigned int, int, int, std::vector<COutput>,
        ...
        f = fMinimizeCoinAge
                ? &CWallet::SelectCoinsMinConfByCoinAge
...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64_t> > &vecSend, CWalletTx &wtxNew, CReserveKey &reservekey,
                                int64_t &nFeeRet, int nSplitBlock, const CCoinControl *coinControl)
{
    int64_t nValue = 0;
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                BOOST_FOREACH (PAIRTYPE(
                                   const CWalletTx *, unsigned int) pcoin,
                               setCoins)
                {
...
bool CWallet::GetStakeWeight2(const CKeyStore &keystore, uint64_t &nMinWeight, uint64_t &nMaxWeight, uint64_t &nWeight,
                              uint64_t &nHoursToMaturity, uint64_t &nAmount)
{
    // Choose coins to use
    int64_t nBalance = GetBalance();
                   ...
                   setCoins)
    {
        ...
        if ((nStakeAge - nCurrentAge) < (60 * 60 * 24 *
                                         2)) // if the age is less than 2 days, report weight as 0 because the stake modifier won't allow for stake yet
            bnCoinDayWeight = 0;
                              ...
                              CTransaction &txNew, CKey &key)
{
                   ...
                   setCoins)
    {
        ...
        if (CheckStakeKernelHash(nBits, block, txindex.pos.nTxPos - txindex.pos.nBlockPos, *pcoin.first,
                                 prevoutStake, txNewTime, nHashDrift, false, hashProofOfStake))
        {
            // Found a kernel
            ...
            uint64_t nTotalSize = pcoin.first->vout[pcoin.second].nValue * (1 + ((txNew.nTime -
                                                                                  block.GetBlockTime()) /
                                                                                 (60 * 60 * 24)) *
            ...
            {
                nTotalSize = pcoin.first->vout[pcoin.second].nValue * (1 + ((txNew.nTime -
                                                                             block.GetBlockTime()) /
                                                                            (60 * 60 * 24)) *
                                                                               (MAX_MINT_PROOF_OF_STAKE_2 /
                                                                                COIN / 365));
            }
...
string
CWallet::SendMoney(CScript scriptPubKey, int64_t nValue, CWalletTx &wtxNew, bool fAskFee, bool fAllowStakeForCharity)
{
...
{
    {
        ...
        if (block.IsProofOfWork() && mapWallet.count(block.vtx[0].GetHash()))
        {
            ...
            printf("    mine:  %d  %d  %" PRId64 "", wtx.GetDepthInMainChain(), wtx.GetBlocksToMaturity(),
                   wtx.GetCredit());
        }
        if (block.IsProofOfStake() && mapWallet.count(block.vtx[1].GetHash()))
        {
            CWalletTx &wtx = mapWallet[block.vtx[1].GetHash()];
            printf("    stake: %d  %d  %" PRId64 "", wtx.GetDepthInMainChain(), wtx.GetBlocksToMaturity(),
                   wtx.GetCredit());
        }
    }
    printf("\n");
