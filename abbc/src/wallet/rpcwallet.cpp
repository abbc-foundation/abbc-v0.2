...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
            ...
            "\nReturns a new alibaba address for receiving payments.\n"
            "If 'account' is specified (recommended), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) The account name for the address to be linked to. if not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "\nResult:\n"
            "\"alibaba address\"    (string) The new alibaba address\n"
            "\nExamples:\n"
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
                     ...
                     "Each key is a ABBC address or hex-encoded public key.\n"
                     "If 'account' is specified, assign address to that account.\n"

                     "\nArguments:\n"
                     "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
                     "2. \"keysobject\"   (string, required) A json array of abbc addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"address\"  (string) abbc address or hex-encoded public key\n"
                     "       ...,\n"
                     "     ]\n"
                     "3. \"account\"      (string, optional) An account to assign the addresses to.\n"

                     "\nResult:\n"
                     "\"abbcaddress\"  (string) A abbc address associated with the keys.\n"
...
Value addredeemscript(const Array& params, bool fHelp)
{
    ...
    pwalletMain->SetAddressBook(innerID, strAccount ,"redeemScript");
    return CBitcoinAddress(innerID).ToString();
}
...
Value listlabels(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
                "listaccounts [minconf=1]\n"
...
Value getunconfirmedbalance(const Array& params, bool fHelp)
{
        if (fHelp || params.size() == 1)
            throw runtime_error(
                ...
                "returns the number of unconfirmations for <txid>"
                );

        Object entry;

        entry.push_back(Pair("result", ValueFromAmount(pwalletMain->GetUnconfirmedBalance())));
        entry.push_back(Pair("error", "null"));
        entry.push_back(Pair("id", "null"));
...
Value abortrescan(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
     ...
     if (!pwalletMain->IsScanning() || pwalletMain->IsAbortingRescan())
       return false;
